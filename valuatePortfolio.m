function [ value ] = valuatePortfolio( optionPortfolio, volSurface, TPortfolio, kPortfolio, r, optionsDelta, s, rSWE, rUSD, maturityT )
    ...
    for j = 1:size(kPortfolio,2)
        for k = 1:size(kPortfolio,1)
            ...
            if k<8
                ...
                value = value + optionPortfolio(k,j)*put(kPortfolio(k,j), d1, d_2(d1, vol, TPortfolio(j)), forwardPrice(s,rSWE,rUSD, 0, maturityT(j,1)), r(i), TPortfolio(j));
            else
                 d1 = d_1(forwardPrice(s,rSWE,rUSD, 0, maturityT(j,1)), kPortfolio(k,j), vol , TPortfolio(j));
                 value = value + optionPortfolio(k,j)*call(kPortfolio(k,j), d1, d_2(d1, vol, TPortfolio(j)), forwardPrice(s,rSWE,rUSD, 0, maturityT(j,1)), r(i), TPortfolio(j));
            end
